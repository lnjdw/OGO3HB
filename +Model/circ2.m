...
Rven    = 5*1;		% [kPa.ms/ml] - venous resistance
%
% discretisation
...
while(j<=ninc)
    ...
    % if plv(j) > part(j)
    %    qav(j) = (plv(j)-part(j))/Rart;
    % else
    %    qav(j) = (plv(j)-part(j))/(10*Rart);
    % end

    qmv(j)  = max(0,(pven(j)-plv(j))/Rven); 
    % if pven(j) > plv(j)
    %    qmv(j) = (pven(j)-plv(j))/Rven;
    % else
    %    qmv(j) = (pven(j)-plv(j))/(100*Rven);
    % end
